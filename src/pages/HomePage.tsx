...
const HomePage: React.FC = () => {
  ...
  const activeProgressionData = activeProgressions.map(id => getProgressionById(id)).filter(progression => progression !== undefined);
  return <Layout title="Calisthenics Tracker">
      <div className="p-4 max-w-lg mx-auto">
        {/* Welcome Section */}
        <section className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Welcome Back!</h2>
          <p className="text-gray-600">
            {lastWorkoutDate ? `Last workout: ${formatDate(lastWorkoutDate)}` : "Let's get your first workout started!"}
          </p>
        </section>
        ...
        <section className="mb-8">
          ...
          <div className="space-y-3">
            ...
            return <Card key={progression.id} className="p-4 hover:shadow-md transition-shadow">
                  ...
                  {nextExercise && <p className="text-xs text-gray-500 mt-1">
                      Next: {nextExercise.name}
                    </p>}
                </Card>;
          })}
            
            {activeProgressionData.length === 0 && <Card className="p-4 text-center">
        ...
        <section>
          ...
          <div className="space-y-3">
            {workoutHistory.slice(0, 3).map(workout => <Card key={workout.id} className="p-4 hover:shadow-md transition-shadow">
            ...
            {workoutHistory.length === 0 && <Card className="p-4 text-center">
